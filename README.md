...
This is a client-side wallet library that can generate private keys from a mnemonic, or import private keys from other BITSTASH wallets.

It can sign transactions locally, and submit the raw transaction data to a remote bitstash node. The blockchain data is provided by the Insight API (which powers https://explorer.bitstash.org/), rather than the raw bitstashd RPC calls.

This library makes it possible to run DApp without the users having to run a full bitstashd node.

> This library is extracted from the official [BITSTASH web wallet](https://github.com/bitstashproject/bitstash-web-wallet).

## Install

```
yarn add bitstashjs-wallet
```

## Running Tests

The bitstashjs-wallet depends on a number of external services. To run the unit tests, you'll need to start the services using [docker-compose](https://docs.docker.com/compose/install/).
...
- Uses satoshi (1e8) as internal units
  - Can represent up to ~90 million BITSTASH accurately.
...
```js
import { networks, generateMnemonic } from "bitstashjs-wallet"
...
The currency unit used is `satoshi`. To convert bitstash to satoshi you should multiply the amount you want with `1e8`.

```js
import { networks } from "bitstashjs-wallet"
...
The ABI encoding for the `burnbabyburn()` invokation is `e179b912`. We'll burn 0.05 bitstash, expressed in unit of satoshi.
...
async function main() {
  ...
  const tx = await wallet.contractSend(contractAddress, encodedData, {
    amount: 0.05 * 1e8, // 0.05 bitstash in satoshi
  })
...
Suppose you want to import the public address `qg3HYD8c4bAVLeEzA9t3Ken3Y3Mni1HZSS`. Use `bitstash-cli` to dump the private key from wallet:
...
const amount = 0.15 * 1e8 // 0.15 BITSTASH
...
```ts
const tx = await wallet.send(toAddress, amount, {
  // rate is 400 satoshi per byte, or  ~0.004 bitstash/KB, as is typical.
  feeRate: 400,
})
```
...
You can decode the raw transaction using `bitstash-cli`:

```
bitstash-cli decoderawtransaction 0100000001a09a921ecb38...
...
```
bitstash-cli decoderawtransaction 0100000001e7b6f2767e8a6...
```
...
```js
import { scrypt } from "bitstashjs-wallet"
...
```js
import { scrypt, params } from "bitstashjs-wallet"
